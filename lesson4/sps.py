...
def shortest_path_search(start, successors, is_goal):
    ...
    while frontier:
        path = frontier.pop(0)
        state_frontier = path[-1]  # most recent state, if state in frontier then it is not goal
        # explore the sucessors of path
        for (state, action) in successors(state_frontier).items():
